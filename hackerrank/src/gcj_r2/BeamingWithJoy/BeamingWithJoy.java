...
public class BeamingWithJoy {
    ...
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new FileReader("src/gcj_r2/BeamingWithJoy/C-large.in"));
        ...
        Files.write(Paths.get("src/gcj_r2/BeamingWithJoy/C-large.out"), sb.toString().getBytes());
    ...
    private static boolean solve(int R, int C, char[][] grid) {
        ...
        for (int beam_i = 0; beam_i < beams.size(); beam_i++) {
            ...
            for (int[] move : moves) {
                ...
                int r = beam.r + m0;
                int c = beam.c + m1;
                while (c >= 0 && r >= 0 && c < C && r < R && grid[r][c] != '#' && grid[r][c] != '-' && grid[r][c] != '|') {
                    // switch dir
                    if (grid[r][c] == '/') {
                        int tmp0 = m0;
                        m0 = -m1;
